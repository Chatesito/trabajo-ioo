...
\begin{tikzpicture}[
    font=\small,
    container/.style={draw, rounded corners=2pt, fill=white, minimum width=3.8cm, minimum height=1.5cm, align=center, thick},
    browser/.style={draw, rounded corners=2pt, fill=primaryblue!20, minimum width=3.4cm, minimum height=0.9cm, align=center, thick},
    vol/.style={draw, cylinder, shape border rotate=90, fill=primaryblue!10, minimum height=1.3cm, minimum width=2.4cm, aspect=0.3, thick},
    flow/.style={-{Latex[length=2.5mm]}, thick},
    port/.style={font=\footnotesize\ttfamily},
    lbl/.style={font=\footnotesize},
    ]

    \node[browser] (browser) {Navegador\\ (usuario)};

    \node[container, below=1.8cm of browser, xshift=-2.6cm] (web) {\textbf{clinica\_web}\\ Django + gunicorn\\ \texttt{:8000}};
    \node[container, right=1.4cm of web] (db) {\textbf{clinica\_db}\\ PostgreSQL 16\\ \texttt{:5432}};
    \node[vol, below=1.4cm of db] (volume) {pgdata};

    \draw[flow] (browser) -- node[lbl, right, pos=0.45] {HTTP \texttt{8000:8000}} (web);
    \draw[flow] (web) -- node[lbl, above] {TCP \texttt{5432}} node[lbl, below] {DNS: \texttt{db}} (db);
    \draw[flow] (db) -- node[lbl, right] {\texttt{/var/lib/postgresql/data}} (volume);

    \begin{pgfonlayer}{background}
        \node[draw, dashed, rounded corners, fill=secondaryblue!8, inner sep=0.45cm, fit=(web)(db)] (net) {};
        \node[above right=-0.05cm and 0cm of net.north west, font=\footnotesize\bfseries\color{secondaryblue}] {Red privada: clinica\_net};

        \node[draw, rounded corners, fill=primaryblue!5, inner sep=0.6cm, fit=(net)(volume)] (podman) {};
        \node[above right=-0.05cm and 0cm of podman.north west, font=\footnotesize\bfseries\color{primaryblue}] {Podman runtime (rootless)};

        \node[draw, thick, rounded corners, fill=gray!8, inner sep=0.55cm, fit=(browser)(podman)] (host) {};
        \node[above right=-0.05cm and 0cm of host.north west, font=\footnotesize\bfseries] {Host (Linux --- tu PC)};
    \end{pgfonlayer}
\end{tikzpicture}
\caption{Arquitectura de contenedores del sistema. El navegador del usuario accede al puerto 8000 del host, que está mapeado al contenedor \texttt{clinica\_web}. Éste se comunica con \texttt{clinica\_db} a través de la red privada \texttt{clinica\_net} usando el nombre DNS \texttt{db}. La base de datos persiste sus archivos en el volumen nombrado \texttt{pgdata}. El puerto 5432 de Postgres NO está expuesto al host.}
\end{figure}
...
La imagen queda disponible en \url{https://hub.docker.com/r/donleonz/clinica-ioo} y cualquier persona puede descargarla y ejecutarla con:
